  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const data = await (
          await axios.get(
            "https://res.creativeparkingsolutions.com/api/admin/getallmenu"
          )
        ).data;
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await (
          await axios.get(
            "https://res.creativeparkingsolutions.com/api/admin/getallitems"
          )
        ).data;
        setItem(data.data);
        console.log(item);
          ...
          <div className="row justify-content-center">
            <div className="col-md-12 text-center" style={{ padding: "0px" }}>
              ...
              <img
                  ...
                  src="https://c4.wallpaperflare.com/wallpaper/794/96/864/pizza-fast-food-top-view-cheese-wallpaper-preview.jpg"
                />
                <div className="centered-text">
                <h1 className="boldtext centerh1 px-3">A House Of Taste</h1>
                </div>
              </div>
            </div>
          </div>
          <ul className="nav nav-pills nav-fill stick">
            {category &&
              ...
              return (
                <>
                  <div className="row productrow" id={`${categorys.Name}`}>
                    <div className="col-xl-12 responsiveness">
                      <h3 className="boldtext ms-2 mt-5 categoryname nomargin">
                        {categorys.Name}
                      </h3>
                      <div className="row centeritems px-5 justify-content-center">
                        {item &&
                          item.map((items) => {
                            return (
                              <>
